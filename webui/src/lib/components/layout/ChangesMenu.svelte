<script lang="ts">
	...
	let validationErrors = $state<
		Array<{ category: string; level: string; message: string; path?: string }>
	>([]);
	...
	function getOperationBadge(operation: EntityChange['operation']): {
		bg: string;
		text: string;
		label: string;
	} {
	...
	function connectToValidationStream(sseUrl: string) {
		...
		es.onmessage = (event) => {
			try {
				...
				if (data.type === 'progress') {
					...
					validationProgress = '';
					validationErrors = [
						{ category: 'System', level: 'ERROR', message: data.message || 'Validation failed' }
					];
		...
		es.onerror = async () => {
			...
			validationProgress = '';
			validationErrors = [
				{ category: 'System', level: 'ERROR', message: 'Lost connection to validation stream' }
			];
			validationErrorCount = 1;
		};
	}
	...
	// Turn a granular property path (e.g. "sizes[0].purchase_links") into a readable label
	// ("Size 1 › purchase links") for the change diff.
	function formatPropertyLabel(property: string): string {
		return property
			.replace(/\bsizes\[(\d+)\]/g, (_, n) => `Size ${Number(n) + 1}`)
			.replace(/\bpurchase_links\[(\d+)\]/g, (_, n) => `Link ${Number(n) + 1}`)
			.split('.')
			.join(' › ')
			.replace(/_/g, ' ');
	}
...
<div class="relative">
	<Button
		...
		title={$hasChanges
			...
			: $hasSubmitted
				? `${$submittedCount} submitted changes`
				: 'No pending changes'}
	>
		...
		{#if $hasChanges}
			<span
				class="flex h-5 min-w-5 items-center justify-center rounded-full bg-destructive px-1 text-xs font-medium text-white"
			>
				{$changeCount}
			</span>
		{:else if $hasSubmitted}
			<span
				class="flex h-5 min-w-5 items-center justify-center rounded-full bg-purple-500 px-1 text-xs font-medium text-white"
			>
				{$submittedCount}
			</span>
		{/if}
	...
	{#if menuOpen}
		...
		<div
			class="absolute top-full right-0 z-50 mt-2 w-96 max-w-[calc(100vw-2rem)] rounded-lg border bg-popover shadow-md"
		>
			<!-- Header -->
			...
			<div class="max-h-96 overflow-y-auto">
							...
							<div class="p-3">
								...
								<div class="flex items-start justify-between gap-2">
									<div class="min-w-0 flex-1">
										<div class="flex items-center gap-2">
											<span
												class="rounded px-1.5 py-0.5 text-xs font-medium {badge.bg} {badge.text}"
											>
												{badge.label}
											</span>
								...
								{#if change.propertyChanges && change.propertyChanges.length > 0}
									...
									>
										...
										{change.propertyChanges.length === 1 ? ' field' : ' fields'} changed
									</Button>
									...
									{#if isExpanded}
										<div class="mt-2 space-y-2 rounded border bg-muted/50 p-2 text-xs">
											{#each change.propertyChanges as propChange}
												<div>
													<span class="font-medium">{formatPropertyLabel(propChange.property)}</span
													>
													{#if propChange.oldValue === undefined}
														<span class="ml-1 text-green-600 dark:text-green-400">added</span>
														<pre
															class="mt-1 overflow-x-auto rounded bg-green-500/10 p-1.5 text-green-700 dark:text-green-400">{formatValue(
																propChange.newValue
															)}</pre>
													{:else if propChange.newValue === undefined}
														<span class="ml-1 text-destructive">removed</span>
														<pre
															class="mt-1 overflow-x-auto rounded bg-destructive/10 p-1.5 text-destructive line-through">{formatValue(
																propChange.oldValue
															)}</pre>
													{:else}
														<div class="mt-1 grid gap-1">
															<pre
																class="overflow-x-auto rounded bg-destructive/10 p-1.5 text-destructive line-through">{formatValue(
																	propChange.oldValue
																)}</pre>
															<pre
																class="overflow-x-auto rounded bg-green-500/10 p-1.5 text-green-700 dark:text-green-400">{formatValue(
																	propChange.newValue
																)}</pre>
														</div>
													{/if}
				...
				{:else}
					<div class="flex flex-col items-center justify-center px-4 py-8 text-center">
						...
						<p class="mt-1 text-xs text-muted-foreground">Changes you make will appear here</p>
					</div>
				{/if}
			</div>
				...
				<div class="border-t">
					<div class="flex items-center justify-between bg-purple-500/5 px-4 py-2">
						<h4 class="text-xs font-semibold text-purple-700 dark:text-purple-400">
							...
							{$submittedCount === 1 ? 'change' : 'changes'} awaiting merge)
						</h4>
					</div>
						...
						{#each submittedStore.getEntries() as entry}
							<div class="px-4 py-3 opacity-75">
								<div class="flex items-center justify-between">
									<div class="flex items-center gap-2">
										<span
											class="rounded bg-purple-500/20 px-1.5 py-0.5 text-xs font-medium text-purple-700 dark:text-purple-400"
										>
											PR #{entry.prNumber}
										</span>
										<span class="text-xs text-muted-foreground">
...
<Modal
	...
	title={$isLocalMode ? 'Save Changes' : 'Submit Changes'}
	onClose={() => {
						...
						{#if validationStatus === 'complete'}
							<div class="mb-2 flex shrink-0 items-center gap-2">
								{#if validationErrorCount > 0}
									<span
										class="rounded-full bg-destructive/10 px-2.5 py-0.5 text-xs font-medium text-destructive"
									>
								...
								{#if validationWarningCount > 0}
									<span
										class="rounded-full bg-yellow-500/10 px-2.5 py-0.5 text-xs font-medium text-yellow-700 dark:text-yellow-400"
									>
							...
							<div class="min-h-0 flex-1 space-y-1.5 overflow-y-auto">
								{#each validationErrors as error}
									<div
										class="rounded border px-2.5 py-1.5 text-xs {error.level === 'ERROR'
							...
							<p class="mt-3 text-xs text-muted-foreground">
								Ideally you should never face issues like these, please <a
									href="https://github.com/OpenFilamentCollective/open-filament-database/issues"
									class="underline hover:text-foreground">submit an issue</a
								> with the above errors in the message body.
							</p>
						...
						<div>
							...
							{:else}
								<p class="mb-3 text-sm text-muted-foreground">
									Write all pending changes to the local data files. This will run validation and
					...
					<div class="flex flex-1 flex-wrap items-center gap-2">
						...
						{:else if validationStatus === 'complete'}
							{#if validationIsValid}
								<span
									class="rounded-full bg-green-500/10 px-2.5 py-1 text-xs font-medium text-green-700 dark:text-green-400"
									>Valid</span
								>
							{:else}
								<span
									class="rounded-full bg-destructive/10 px-2.5 py-1 text-xs font-medium text-destructive"
								>
									{validationErrorCount}
									{validationErrorCount === 1 ? 'error' : 'errors'}{#if validationWarningCount > 0}, {validationWarningCount}
										{validationWarningCount === 1 ? 'warning' : 'warnings'}{/if}
								</span>
							{/if}
						{:else if validationStatus === 'error'}
							<span
